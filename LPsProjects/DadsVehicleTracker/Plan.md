...
2. ~~Survive reboots~~ done, except: (0) **S4U re-registration** (run whether logged on or not + at-startup trigger) needs an elevated PowerShell — do it next time someone is at the PC to click UAC; (a) confirm Docker Desktop → Settings → General → "Start Docker Desktop when you sign in" is ticked; (b) decide on Windows auto-login or accept signing in after reboots; (c) **schedule weekly `certbot-renew` + `docker compose restart fleet-telemetry`** (cert expires 2026-12-11) — not yet done.
